...
#   [재확인 2026-09-17, fire_audit.py / fire_audit2.py] **2 가 유일하게 작동하는 값이다.**
#     누름=0  발사 0발                    (발사 없음)
#     누름=1  **시드 5개 중 3개에서 0발**   위상에 따라 ROM 이 못 읽는다. 점수 40~70
#     누름=2  전 시드 정상                110~152발, 점수 1,080~2,050
#     누름=3  발사 1발                    뗌이 1프레임뿐이라 재발사가 안 된다
#     누름=4  발사 1발                    = 14문서의 그 버그 상태
#   무작위 no-op 시작 때문에 결정 경계와 ROM 폴링 위상이 매번 어긋난다.
#   1프레임 누름은 그 위상에 걸리면 통째로 무시된다 — **여유가 없다.**
#   위아래가 다 막혀 있으므로 이 값은 **바꾸지 말 것.**
